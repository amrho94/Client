-- Ported from CuteVisuals.java; the game adapter supplies bed events.
return function(neon, adapter)
 local runService = game:GetService('RunService')
 local debris = game:GetService('Debris')
 local module
 local options = {}
 local folder = Instance.new('Folder')
 folder.Name = 'NeonCuteVisuals'
 ...
 local unit = 2.5 -- Minecraft blocks to Roblox studs.
 ...
 module=neon:Module('Vision', {Name='CuteVisuals',DisplayName='Dream Particles',Tooltip='Floating pink hearts, drifting dots, pastel bursts and rainbow celebrations. Ported from CuteVisuals.java.',Function=function(enabled)
  clear()
  if enabled then
 ...
 neon:Clean(function()clear();folder:Destroy()end)
 return module
end
