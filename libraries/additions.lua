...
return function(ctx)
	...
	local function start()
		silent = ctx:find('SilentAim', 'offense')
		...
		-- Stop MagicBullet first; both modules hook spawn_bullet.
		if silent then
...
return function(ctx)
	...
	local function gameload()
		...
		-- Try the usual entrypoints if the tree request fails.
		if #files == 0 then
			local candidates = {
				root..'/init.lua',
				'src/games/'..id..'.lua'
...
sources['src/modules/movement/fakelag.lua'] = [==[
return function(ctx)
	local mod
	local meth
	local ping
...
sources['src/modules/movement/lagswitch.lua'] = [==[
return function(ctx)
	local mod
	local meth
	local mode
...
sources['src/modules/movement/manifest.lua'] = [==[
return {
	files = {
		'fakelag.lua',
		'lagswitch.lua'
...
return function(ctx)
	...
	mod = ctx:module('offense', {
		...
		func = function(on)
			if on then
				paint()
				silent = ctx:find('SilentAim', 'offense') or ctx:find('SilentAim')
...
return {categories = {'offense', 'motion'}}
]==]
...
return function(neon)
	...
	local ctx = {
		...
		cats = {names = {offense='Offense', motion='Motion', vision='Vision', utility='Utility', world='World', scripts='Scripts'},
			order = {'offense', 'motion', 'vision', 'utility', 'world', 'scripts'}},
