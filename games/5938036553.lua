...
run(function()
	...
	-- Use these events instead of adding another SPAWN_FPV_SOL_BULLET hook.
...
-- Some hitboxes have no Head name; fall back to the hit position.
...
run(function()
	neon.Modules.Speed:AddMode('Legit', function(options, moveDirection, dt)
		...
		-- Use an offset so the extra speed does not compound each frame.
...
run(function()
	...
	local function getClosestTarget(origin)
		...
		for _, ent in ipairs(entitylib.List) do
			...
			-- Ignore the target when checking for walls.
...
run(function()
	...
	local function selectAimTarget(origin, selectedMode, selection)
		...
		local points, saved = {}, {}
		-- Use the same aim point for FOV, wall checks and the bullet solver.
		local ok, entity = pcall(function()
			for _, ent in ipairs(entitylib.List) do
				...
				if finite(pos) then
					...
					-- Selectors need Part even for stale entities; reject them below.
...
run(function()
	...
	GrenadeTP = client:Module('Motion', {
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	GunModifications = client:Module('Motion', {
...
run(function()
	-- Lighting and post-processing only, not hardware ray tracing.
	...
	RTXShaders = client:Module('Vision', {
		Name = 'RTXShaders',
		Tooltip = 'Lighting presets with fog, bloom and color adjustments. Not ray tracing.',
...
run(function()
	...
	local function startVisual()
		...
		-- Only change the rendered bone, not camera or simulation state.
		runService:BindToRenderStep(renderName, Enum.RenderPriority.Last.Value + 100, function(dt)
			...
			-- Root_M starts at roughly 90 degrees yaw and -90 degrees roll.
...
run(function()
	...
	local function restore()
		if model and model.Parent and original and applied then
			-- Do not restore an old pose if the game has already animated a new one.
	...
	local function render()
		...
		-- Check proximity so a world model cannot be mistaken for the viewmodel.
	...
	GunViewmodel = client:Module('Vision', {
		Name = 'GunViewmodel',
		Function = function(callback)
			...
			-- Clear before simulation; apply once after rendering to avoid drift.
...
run(function()
	...
	local function findGunAssembly(model)
		...
		-- Fallback for hidden client-side welds.
	...
	local function applyGun(now)
		...
		-- PBR maps can mask ForceField; save them until the override is disabled.
	...
	GunChanger = client:Module('Vision', {
		Name = 'GunChanger',
		Function = function(callback)
			if callback then
				task.defer(function()
					...
					GunChanger:Clean(runService.RenderStepped:Connect(function()
						...
						-- Type 2 is melee; leave the knife alone.
		...
		Tooltip = 'Gives your gun a rainbow ForceField finish.'
	})
...
run(function()
	...
	local function collectAssetModels(objects)
		...
		-- Assets can contain a single part without a Model.
		if #names == 1 then
			for _, root in ipairs(objects) do
	...
	local function createTemplate(obj)
		...
		-- Center the pivot on the visible geometry.
	...
	-- Exclude hands, arms and the reticle from the knife bounds.
	...
	local function applyAutoScale()
		...
		-- Match the original knife size before applying the scale setting.
	...
	local function updateKnife()
		...
		-- Hide the knife while a gun is equipped; keep it for later.
		if not gun or gun.type ~= 2 then
			target = nil
			targetBoundsCF = nil
			targetBoundsSize = nil
		...
		-- Rescan when the cached geometry expires or workspace.Model changes.
		if newTarget ~= target
			or not target
		...
		-- Follow the live part; cached bounds make the knife trail behind.
	...
	CustomKnife = client:Module('Vision', {
		Name = 'CustomKnife',
		Function = function(callback)
			if callback then
		...
		Tooltip = 'Replaces the knife with an asset model. Adjust its size and offset below.'
	})
...
run(function()
	...
	local function applyTheme(name)
		...
		-- Reset sky settings before applying another preset.
		resetSky(activeSky)
	...
	SkyThemes = client:Module('Vision', {
		Name = 'SkyThemes',
		Function = function(callback)
			if callback then
				storeGameSkies()
...
run(function()
	...
	Fireflies = client:Module('Vision', {
		Name = 'Fireflies',
		Function = function(callback)
			if callback then
				rebuild()
...
run(function()
	...
	function Scene:track(obj, delay, duration, fn)
		...
		-- Set the initial pose before adding trails to avoid a flash at the origin.
	...
	function Scene:ghosts(count)
		...
		-- Use fallback body parts if the ragdoll is already gone.
		if #result == 0 then
	...
	local function makeScene(c, data)
		...
		-- Unknown saved modes use the default.
	...
	local function consumeDeath(c)
		...
		-- Only deaths following a recent local hit are cached.
	...
	local function createController(kill)
		...
		menu[#menu + 1] = 'Random'
		c.Module = client:Module('Vision', {
			Name = kill and 'KillEffects' or 'HitEffects',
			Tooltip = kill and 'Plays an effect when you get a kill.'
				or 'Plays an effect when you hit a player.',
			Function = function(enabled)
				c.Active = enabled
				reset(c)
		...
		c.ColorMode = module:CreateDropdown({Name = 'Color Mode', List = {'Theme', 'Style', 'Custom', 'Pastel', 'Target', 'Rainbow'}, Default = 'Theme'})
...
run(function()
	...
	local function primeHealth()
		...
		updateHealthUI()
		-- Initialize health for effects that were enabled after the dummy.
	...
	local function testKill(headshot)
		...
		task.delay(0.022, function()
			...
			-- Keep the body briefly so clone effects can copy it.
	...
	FakePlayer = client:Module('Vision', {
		Name = 'FakePlayer',
		Function = function(callback)
			if callback then
				buildDummy()
...
run(function()
	...
	-- Use a workspace file, e.g. sounds/headshot.ogg.
	...
	HeadshotSound = client:Module('Vision', {
		Name = 'HeadshotSound',
		Function = function(callback)
			if callback then
				table.clear(pending)
...
run(function()
	...
	local function addESP(v)
		...
		local adornee = v.model.PrimaryPart or v.model:FindFirstChildWhichIsA('BasePart')
		if not adornee then return end
		local col = Color3.fromHSV(Color.Hue, Color.Sat, Color.Value)
		local data = krsrender:Tag(Folder, adornee, 'GRENADE', {
			Name = 'KrsGrenade', Color = krsrender.Palette.Text, TextSize = 10,
			Transparency = Background.Enabled and (1 - Color.Opacity) or 1,
			Offset3D = Vector3.new(0, 1.4, 0), Offset = 0.16
		})
		krsrender.Objects[data.Accent] = nil
		krsrender.Objects[data.Status] = nil
		...
		v.model.Destroying:Connect(function()
			...
			if Reference[v.model] then
				Reference[v.model].Billboard:Destroy()
				Reference[v.model] = nil
			end
		end)
	end
	
	GrenadeESP = client:Module('Vision', {
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
	...
	Background = GrenadeESP:Setting({Type='toggle', 
		...
		Function = function(callback)
			...
			for _, data in Reference do
				data.Root.BackgroundTransparency = callback and (1 - Color.Opacity) or 1
				data.Stroke.Transparency = callback and 0.68 or 1
			end
		end,
		Default = true
	})
	Color = GrenadeESP:Setting({Type='color', 
		...
		Function = function(hue, sat, val, opacity)
			for _, data in Reference do
				...
				data.Accent.BackgroundColor3 = col
				data.Status.BackgroundColor3 = col
				data.Root.BackgroundTransparency = Background.Enabled and (1 - opacity) or 1
			end
		end,
		Darker = true
	})
...
run(function()
	...
	NoHurtCam = client:Module('Vision', {
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	VisualComfort = client:Module('Vision', {
		Name = 'VisualComfort',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = client:Module('Vision', {
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = client:Module('Utility', {
		Name = 'AutoRespawn',
		Function = function(callback)
			cancelPending()
			if callback then
...
run(function()
	...
	PickupRange = client:Module('Utility', {
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	local function getTracerColor()
		return ThemeColor.Enabled and krsrender:Accent(0.22) or Color3.fromHSV(Color.Hue, Color.Sat, Color.Value)
	end
	
	BulletTracers = client:Module('Vision', {
		Name = 'BulletTracers',
		Function = function(callback)
			if callback then 
				BulletTracers:Clean(frontlines.LocalBulletEvent.Event:Connect(function(id, btype, origin, velocity)
					if DrawingToggle.Enabled then 
						...
						obj.Color = getTracerColor()
						obj.Thickness = Width.Value
						...
						obj.Name = 'KrsShotTrace'
						obj.Size = Vector3.new(Width.Value / 65, Width.Value / 65, 1000)
		...
		Tooltip = 'Shows bullet trails. Use the theme color or pick your own.'
	})
	...
	ThemeColor = BulletTracers:Setting({Type='toggle',
		Name = 'Theme Accent',
		Default = true,
		Function = function(callback)
			if Color.Object then Color.Object.Visible = not callback end
		end
	})
	Color.Object.Visible = not ThemeColor.Enabled
	Width = BulletTracers:Setting({Type='slider', Name = 'Width', Min = 0.5, Max = 3, Default = 1.15, Decimal = 100})
...
run(function()
	...
	BulletNetSpy = client:Module('Utility', {
		Name = 'BulletNetSpy',
...
run(function()
	...
	local function movementFor(root, ent, dt)
		...
		-- The tangent handles strafing; radial correction keeps the requested radius.
	...
	local function stepTargetStrafe(dt)
		...
		if displacement and not hit and ground then
			-- Frontlines moves its soldier root, not PlayerModule.moveFunction.
...
run(function()
	...
	FrontlinesFly = client:Module('Motion', {
		Name = 'FrontlinesFly',
		Function = function(callback)
			if not callback then return end
			...
			FrontlinesFly:Clean(function()
				...
				for _, setting in preset do
					local option = fly.Options[setting[1]]
					-- Do not overwrite settings changed during the pulse.
...
run(function()
	...
	AmmoHUD = client:Module('Vision', {
		...
		Function = function(callback)
			...
			krsrender.Objects[card.Accent] = nil
			krsrender.Objects[card.Status] = nil
			root.Visible = false
			AmmoHUD:Clean(root)
			...
			caption.Position = UDim2.fromOffset(20, 0)
			caption.Size = UDim2.fromOffset(48, 38)
			caption.FontFace = Font.fromEnum(Enum.Font.GothamMedium)
			caption.Text = 'AMMO'
			caption.TextColor3 = krsrender.Palette.Muted
			caption.TextSize = 9
			...
			countLabel.Position = UDim2.fromOffset(70, 0)
			countLabel.Size = UDim2.fromOffset(48, 38)
			countLabel.FontFace = Font.fromEnum(Enum.Font.GothamBold)
			countLabel.Text = '--'
			countLabel.TextColor3 = krsrender.Palette.Text
			countLabel.TextSize = TextSize.Value
			countLabel.TextXAlignment = Enum.TextXAlignment.Right
			countLabel.Parent = root

			local separator = Instance.new('Frame')
			separator.Name = 'Separator'
			separator.BackgroundColor3 = krsrender.Palette.Border
			separator.BackgroundTransparency = 0.45
			separator.BorderSizePixel = 0
			separator.Position = UDim2.fromOffset(126, 10)
			separator.Size = UDim2.fromOffset(1, 18)
			separator.Parent = root

			local reserveLabel = Instance.new('TextLabel')
			reserveLabel.Name = 'Reserve'
			reserveLabel.BackgroundTransparency = 1
			reserveLabel.Position = UDim2.fromOffset(135, 0)
			reserveLabel.Size = UDim2.new(1, -145, 1, 0)
			reserveLabel.FontFace = Font.fromEnum(Enum.Font.GothamMedium)
			reserveLabel.Text = '--'
			reserveLabel.TextColor3 = krsrender.Palette.Muted
			reserveLabel.TextSize = 11
			reserveLabel.TextXAlignment = Enum.TextXAlignment.Left
			reserveLabel.Parent = root
			...
			local function update()
				...
				local state = globals and globals.cli_state
				root.Visible = not not (entitylib.isAlive and state and state.state == main.cli_state_t.COMBAT
				...
				local low = count <= LowAmmo.Value
				local stateColor = low and krsrender.Palette.Warning or krsrender:Accent(0.2)
				countLabel.Text = count == 0 and '0' or tostring(count)
				countLabel.TextColor3 = low and krsrender.Palette.Warning or krsrender.Palette.Text
				countLabel.TextSize = TextSize.Value
				reserveLabel.Text = reserve
				reserveLabel.Visible = ShowReserve.Enabled
				separator.Visible = ShowReserve.Enabled
				card.Accent.BackgroundColor3 = stateColor
				card.Status.BackgroundColor3 = stateColor
				root.Position = UDim2.fromScale(0.5, VerticalPosition.Value / 100)
				root.BackgroundTransparency = Background.Enabled and 0.12 or 1
				card.Stroke.Transparency = Background.Enabled and 0.68 or 1
			end
			local elapsed = 0
		...
		Tooltip = 'Shows ammo while a gun is equipped. Hidden in menus and after death.'
	})
...
run(function()
	...
	LowHealthAlert = client:Module('Utility', {
		Name = 'LowHealthAlert',
		Function = function(callback)
			if not callback then return end
			...
			LowHealthAlert:Clean(runService.Heartbeat:Connect(function(dt)
				...
				-- Wait for health to recover before warning again.
...
run(function()
	...
	local Interval
	local elapsed = 0
	BreakReminder = client:Module('Utility', {
		Name = 'BreakReminder',
		Function = function(callback)
			elapsed = 0
			if not callback then return end
