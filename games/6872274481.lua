...
if not neon.guiscale and neon.gui and neon.gui:FindFirstChild('ScaledGui') then
	neon.guiscale = neon.gui.ScaledGui:FindFirstChildOfClass('UIScale') or {Scale = 1}
end
...
local uipallet = neon.Libraries.uipallet
local tween = neon.Libraries.tween
local color = neon.Libraries.color
...
local getcustomasset = neon.Libraries.getneonasset or neon.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('neon/assets/new/blur.png')
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		neonEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						neonEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Neon', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				neonEvents.InventoryChanged:Fire()
			end

			if newinv.inventory.items ~= oldinv.inventory.items then
				neonEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not neon.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			neon:Clean(connection:Connect(function(...)
				neonEvents[event]:Fire(...)
			end))
		end)
	end

	neon:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		neonEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		neon:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or neon.Loaded == nil
	...
	neon:Clean(neonEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	neon:Clean(neonEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	neon:Clean(neonEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until neon.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
run(function()
	...
	AutoClicker = neon:Module('Offense', {
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TriggerBot = neon:Module('Offense', {
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if neon.Modules.Fly.Enabled or neon.Modules.InfiniteFly.Enabled or neon.Modules.LongJump.Enabled then
	...
	Fly = neon:Module('Motion', {
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(neonEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = neon.Libraries.auraanims, tick()
	local AttackRemote
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function(_, _, dir)
			LongJump:Clean(neonEvents.CatPounce.Event:Connect(function()
	...
	LongJump = neon:Module('Motion', {
		Name = 'LongJump',
		Function = function(callback)
			frictionTable.LongJump = callback or nil
			updateVelocity()
			if callback then
				LongJump:Clean(neonEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(neonEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	local ProjectileAimbot = neon:Module('Motion', {
...
run(function()
	...
	ProjectileAura = neon:Module('Motion', {
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
	...
	Health = neon:Module('Vision', {
		...
		Function = function(callback)
			if callback then
				...
				Health:Clean(label)
				Health:Clean(neonEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	AutoBalloon = neon:Module('Utility', {
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPlay = neon:Module('Utility', {
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(neonEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(neonEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = neon:Module('Utility', {
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(neonEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(neonEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(neonEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = neon:Module('Utility', {
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	PickupRange = neon:Module('Utility', {
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ShopTierBypass = neon:Module('Utility', {
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function staffFunction(plr, checktype)
		...
		elseif Mode.Value == 'AutoConfig' then
			...
			for i, v in neon.Modules do
				if not (table.find(safe, i) or v.Category == 'Vision') then
					if v.Enabled then
						v:Toggle()
					end
					v:SetBind('')
	...
	local function playerAdded(plr)
		...
		elseif getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and neon.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	StaffDetector = neon:Module('Utility', {
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
	...
	task.spawn(function()
		repeat task.wait(1) until neon.Loaded or neon.Loaded == nil
		if neon.Loaded and not StaffDetector.Enabled then
...
run(function()
	TrapDisabler = neon:Module('Utility', {
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoSuffocate = neon:Module('World', {
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
	...
	BedPlates = neon:Module('World', {
		...
		Function = function(callback)
			if callback then
				...
				BedPlates:Clean(neonEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(neonEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
	...
	BedProtector = neon:Module('World', {
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						Image = getcustomasset('neon/assets/new/blur.png'),
...
run(function()
	...
	ArmorSwitch = neon:Module('Player', {
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
	...
	AutoBuy = neon:Module('Player', {
		...
		Function = function(callback)
			if callback then
				...
				local lastupgrades
				AutoBuy:Clean(neonEvents.InventoryAmountChanged.Event:Connect(function()
...
run(function()
	...
	AutoConsume = neon:Module('Player', {
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(neonEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
				AutoConsume:Clean(neonEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		local surface = Color3.fromRGB(12, 17, 23)
		local raised = Color3.fromRGB(20, 28, 36)
		local muted = Color3.fromRGB(143, 156, 168)
		local text = Color3.fromRGB(237, 242, 246)
		local function roundGui(obj, radius)
			local c = Instance.new('UICorner'); c.CornerRadius = UDim.new(0, radius or 7); c.Parent = obj
		end
		local function border(obj, alpha, width)
			local st = Instance.new('UIStroke'); st.Color = Color3.fromRGB(255,255,255); st.Transparency = alpha or 0.88; st.Thickness = width or 1; st.Parent = obj; return st
		end
		local function accent() return neon:GetThemeColor(0.12) end

		local window = Instance.new('Frame')
		window.Name = 'NeonHotbarEditor'
		window.Size = UDim2.fromOffset(690, 448)
		window.Position = UDim2.fromScale(0.5, 0.5)
		window.AnchorPoint = Vector2.new(0.5, 0.5)
		window.BackgroundColor3 = surface
		window.BackgroundTransparency = 0.04
		window.BorderSizePixel = 0
		window.Visible = false
		window.ZIndex = 220
		window.Parent = neon.gui.ScaledGui
		roundGui(window, 10); border(window, 0.82)

		local header = Instance.new('Frame'); header.Size = UDim2.new(1,0,0,54); header.BackgroundColor3 = Color3.fromRGB(9,13,18); header.BackgroundTransparency = 0.1; header.BorderSizePixel = 0; header.ZIndex = 221; header.Parent = window
		local title = Instance.new('TextLabel'); title.BackgroundTransparency = 1; title.Position = UDim2.fromOffset(18,0); title.Size = UDim2.new(1,-100,1,0); title.Text = 'Neon  /  Hotbar layout'; title.TextColor3 = text; title.TextSize = 15; title.FontFace = uipallet.FontSemiBold or uipallet.Font; title.TextXAlignment = Enum.TextXAlignment.Left; title.ZIndex = 222; title.Parent = header
		local close = Instance.new('TextButton'); close.AutoButtonColor=false; close.BackgroundColor3=raised; close.BackgroundTransparency=.15; close.Position=UDim2.new(1,-42,0,13); close.Size=UDim2.fromOffset(28,28); close.Text='×'; close.TextColor3=muted; close.TextSize=19; close.FontFace=uipallet.Font; close.ZIndex=223; close.Parent=header; roundGui(close,6)
		close.MouseButton1Click:Connect(function() window.Visible=false; neon.gui.ScaledGui.ClickGui.Visible=true end)

		local search = Instance.new('TextBox'); search.ClearTextOnFocus=false; search.BackgroundColor3=raised; search.BackgroundTransparency=.12; search.Position=UDim2.fromOffset(18,72); search.Size=UDim2.fromOffset(654,32); search.Text=''; search.PlaceholderText='Search items'; search.TextColor3=text; search.PlaceholderColor3=muted; search.TextSize=11; search.FontFace=uipallet.Font; search.TextXAlignment=Enum.TextXAlignment.Left; search.ZIndex=222; search.Parent=window; roundGui(search,6); border(search,.9)
		local sp = Instance.new('UIPadding'); sp.PaddingLeft=UDim.new(0,10); sp.PaddingRight=UDim.new(0,10); sp.Parent=search

		local items = Instance.new('ScrollingFrame'); items.BackgroundTransparency=1; items.BorderSizePixel=0; items.Position=UDim2.fromOffset(18,116); items.Size=UDim2.fromOffset(654,238); items.CanvasSize=UDim2.new(); items.AutomaticCanvasSize=Enum.AutomaticSize.Y; items.ScrollBarThickness=3; items.ScrollBarImageColor3=accent(); items.ZIndex=221; items.Parent=window
		local grid = Instance.new('UIGridLayout'); grid.CellSize=UDim2.fromOffset(48,48); grid.CellPadding=UDim2.fromOffset(6,6); grid.FillDirectionMaxCells=11; grid.SortOrder=Enum.SortOrder.LayoutOrder; grid.Parent=items

		local slots = Instance.new('Frame'); slots.BackgroundTransparency=1; slots.Position=UDim2.fromOffset(18,372); slots.Size=UDim2.fromOffset(654,58); slots.ZIndex=221; slots.Parent=window
		local slotLayout=Instance.new('UIListLayout'); slotLayout.FillDirection=Enum.FillDirection.Horizontal; slotLayout.Padding=UDim.new(0,7); slotLayout.HorizontalAlignment=Enum.HorizontalAlignment.Center; slotLayout.VerticalAlignment=Enum.VerticalAlignment.Center; slotLayout.Parent=slots
		local slotObjects={}
		for i=1,9 do
			local slot=Instance.new('TextButton');slot.Name='Slot'..i;slot.AutoButtonColor=false;slot.BackgroundColor3=raised;slot.BackgroundTransparency=.12;slot.Size=UDim2.fromOffset(50,50);slot.Text='';slot.ZIndex=222;slot.Parent=slots;roundGui(slot,6)
			local st=border(slot,i==1 and .25 or .9,i==1 and 1.4 or 1);st.Name='Edge';st.Color=i==1 and accent()or Color3.new(1,1,1)
			local image=Instance.new('ImageLabel');image.Name='Image';image.BackgroundTransparency=1;image.AnchorPoint=Vector2.new(.5,.5);image.Position=UDim2.fromScale(.5,.5);image.Size=UDim2.fromOffset(31,31);image.ZIndex=223;image.Parent=slot
			slotObjects[i]={Button=slot,Image=image,Stroke=st}
			slot.MouseButton1Click:Connect(function()
				selectedslot=i
				for n,data in slotObjects do local active=n==i;data.Stroke.Color=active and accent()or Color3.new(1,1,1);data.Stroke.Transparency=active and .25 or .9;data.Stroke.Thickness=active and 1.4 or 1 end
			end)
			slot.MouseButton2Click:Connect(function()
				local obj=self.Hotbars[self.Selected];if not obj then return end
				obj.Hotbar[tostring(i)]=nil;image.Image='';if obj.Slots and obj.Slots[i]then obj.Slots[i].Image=''end
			end)
		end

		local function refreshSlots()
			local obj=self.Hotbars[self.Selected]
			for i,data in slotObjects do
				local id=obj and obj.Hotbar[tostring(i)]
				data.Image.Image=id and bedwars.getIcon({itemType=id},true)or''
			end
		end
		self.RefreshEditor=refreshSlots

		local function createItem(id,image)
			local button=Instance.new('TextButton');button.Name=id;button.AutoButtonColor=false;button.BackgroundColor3=raised;button.BackgroundTransparency=.16;button.Text='';button.ZIndex=222;button.Parent=items;roundGui(button,6);border(button,.92)
			local icon=Instance.new('ImageLabel');icon.BackgroundTransparency=1;icon.AnchorPoint=Vector2.new(.5,.5);icon.Position=UDim2.fromScale(.5,.5);icon.Size=UDim2.fromOffset(31,31);icon.Image=image or'';icon.ZIndex=223;icon.Parent=button
			button.MouseButton1Click:Connect(function()
				local obj=self.Hotbars[self.Selected];if not obj then return end
				obj.Hotbar[tostring(selectedslot)]=id;slotObjects[selectedslot].Image.Image=image or'';if obj.Slots and obj.Slots[selectedslot]then obj.Slots[selectedslot].Image=image or''end
			end)
		end
		local popular={'diamond_sword','diamond_pickaxe','diamond_axe','shears','wood_bow','wool_white','fireball','apple','iron','gold','diamond','emerald'}
		local function rebuild(query)
			for _,child in items:GetChildren()do if child:IsA('TextButton')then child:Destroy()end end
			query=tostring(query or''):lower()
			if query==''then
				for _,id in popular do local meta=bedwars.ItemMeta[id];if meta and meta.image then createItem(id,meta.image)end end
			else
				local found={};for id,meta in bedwars.ItemMeta do if meta.image and id:lower():find(query,1,true)then table.insert(found,id)end end;table.sort(found);for i=1,math.min(#found,132)do local id=found[i];createItem(id,bedwars.ItemMeta[id].image)end
			end
		end
		search:GetPropertyChangedSignal('Text'):Connect(function()rebuild(search.Text)end);rebuild('')
		function self:OpenEditor() refreshSlots();neon.gui.ScaledGui.ClickGui.Visible=false;window.Visible=true end
		table.insert(neon.Windows,window)
		return window
	end

	neon.Components.HotbarList=function(optionsettings,children,api)
		...
		local add=Instance.new('TextButton');add.AutoButtonColor=false;add.BackgroundColor3=raised;add.BackgroundTransparency=.08;add.Position=UDim2.new(1,-68,0,7);add.Size=UDim2.fromOffset(58,25);add.Text='+ layout';add.TextColor3=text;add.TextSize=9;add.FontFace=uipallet.Font;add.Parent=root;roundGui(add,5)
		local list=Instance.new('Frame');list.BackgroundTransparency=1;list.Position=UDim2.fromOffset(8,42);list.Size=UDim2.new(1,-16,0,0);list.Parent=root
		local layout=Instance.new('UIListLayout');layout.Padding=UDim.new(0,5);layout.SortOrder=Enum.SortOrder.LayoutOrder;layout.Parent=list
		layout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()list.Size=UDim2.new(1,-16,0,layout.AbsoluteContentSize.Y);root.Size=UDim2.new(1,-2,0,50+layout.AbsoluteContentSize.Y)end)
		optionapi.Object=root;optionapi.Window=CreateWindow(optionapi)

		local function selectHotbar(index)
			optionapi.Selected=math.clamp(index,1,math.max(#optionapi.Hotbars,1))
			for i,data in optionapi.Hotbars do if data.Object then data.Object.BackgroundColor3=i==optionapi.Selected and neon:GetThemeColor(.12):Lerp(surface,.78)or raised;data.Edge.Color=i==optionapi.Selected and neon:GetThemeColor(.12)or Color3.new(1,1,1);data.Edge.Transparency=i==optionapi.Selected and .55 or .94 end end
			if optionapi.RefreshEditor then optionapi:RefreshEditor()end
		end
		function optionapi:AddHotbar(data)
			local hotbardata={Hotbar=type(data)=='table'and table.clone(data)or{},Slots={}}
			table.insert(self.Hotbars,hotbardata);local index=#self.Hotbars
			local row=Instance.new('TextButton');row.AutoButtonColor=false;row.BackgroundColor3=raised;row.BackgroundTransparency=.08;row.Size=UDim2.new(1,0,0,42);row.Text='';row.Parent=list;roundGui(row,5)
			local rowEdge=Instance.new('UIStroke');rowEdge.Color=Color3.new(1,1,1);rowEdge.Transparency=.94;rowEdge.Parent=row;hotbardata.Object=row;hotbardata.Edge=rowEdge
			local dot=Instance.new('Frame');dot.BackgroundColor3=neon:GetThemeColor(index*.04);dot.BorderSizePixel=0;dot.Position=UDim2.fromOffset(8,17);dot.Size=UDim2.fromOffset(7,7);dot.Parent=row;roundGui(dot,4)
		...
		function optionapi:Save(savetab)local hotbars={};for _,v in self.Hotbars do table.insert(hotbars,v.Hotbar)end;savetab.HotbarList={Selected=self.Selected,Hotbars=hotbars}end
	...
	AutoHotbar = neon:Module('Player', {
		...
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(neonEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = neon:Module('Player', {
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedBreakEffect = neon:Module('Vision', {
		Name = 'Bed Break Effect',
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(neonEvents.BedwarsBedBreak.Event:Connect(function(data)
...
run(function()
	...
	local Crosshair = neon:Module('Vision', {
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = neon:Module('Vision', {
...
run(function()
	...
	ReachDisplay = neon:Module('Vision', {
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SoundChanger = neon:Module('Vision', {
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	WinEffect = neon:Module('Vision', {
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
				WinEffect:Clean(neonEvents.MatchEndEvent.Event:Connect(function()
...
run(function()
	...
	BetterSpeed = neon:Module('Motion', {
		Name = 'Speed +',
		Function = function(callback)
...
run(function()
	...
	local function loadPack()
		...
		if not success or not result or not result[1] then
			neon:Notify('TexturePacks', 'Failed to load '..tostring(pack1.Value), 5, 'alert')
			return false
		end

		importedPack = result[1]
		importedPack.Name = 'NeonTexturePack'
	...
	TexturePacks = neon:Module('Vision', {
		Name = 'TexturePacks',
...
run(function()
	...
	InfiniteJump = neon:Module('Utility', {
		Name = 'InfiniteJump',
		Function = function(callback)
			if callback then
